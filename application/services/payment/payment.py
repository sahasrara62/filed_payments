    ...
    def verify_input(self, **kwargs):
        ...
        if (
            not type(
                ...
                and validate_card(kwargs["CreditCardNumber"])
            )
            or not len(kwargs["CreditCardNumber"]) == 16
        ):
        ...
        if kwargs.get("SecurityCode", None):
            if (
                not (
                    type(kwargs.get("SecurityCode", None)) == str
                    and len(kwargs.get("SecurityCode", None)) == 3
                )
                or not kwargs.get("SecurityCode", None).isdigit()
            ):
                print("security code error")
                return False

        if (
            not datetime.datetime.strptime(kwargs["ExpirationDate"], "%Y/%m/%d")
